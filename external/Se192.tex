...
\usepackage{amssymb}
    ...
    \pgfplotsset{hollowdot/.style={color=firstcolor,fill=white,only marks,mark=*, mark size=3pt}}
    \pgfplotsset{soliddot/.style={color=firstcolor,fill=firstcolor,only marks,mark=*, mark size=3pt}}
...
\begin{document}
      \begin{tikzpicture}
                \begin{axis}[
                  ...
                  \addplot+ [leftarrow,domain=-3:-2.02]{1.5-0.5*x};
                  \addplot[hollowdot] coordinates{(-2,2.5)};
                  \addplot [soliddot] coordinates{(-2,1)};
                  \addplot [firstcurvestyle,-,domain=-1.98:-1.02]{1.5-0.5*x};
                  \addplot [hollowdot] coordinates{(-1,2)};
%                  \addplot [hollowdot] coordinates{(-1,1)};
                  \addplot [soliddot] coordinates{(1,2)};                                    
                  \addplot [firstcurvestyle,-,domain=-0.98:0.98]{2*x^2};
                  \addplot [firstcurvestyle,rightarrow,domain=1.02:3]{6-2*x};
                  %\addplot[soliddot] coordinates{(2,2)};
%                  \addplot[soliddot] coordinates{(2,3)};
                  \addplot[hollowdot] coordinates{(1,4)};
                \end{axis}
              \end{tikzpicture}
\end{document}